...
#[account]
pub struct Config {
    /// Admin authority permitted to add/remove issuers. Held separately from the
    /// program upgrade authority (governance in deploy notes, THREAT-MODEL T9).
    pub admin: Pubkey,
...
impl Config {
    // 8 discriminator + 32 admin + 4 vec len + (32 * MAX_ISSUERS) + 1 bump
    pub const MAX_SIZE: usize = 8 + 32 + 4 + (32 * MAX_ISSUERS) + 1;
...
impl Escrow {
    // 8 disc + 32*3 (payer,recipient,mint) + 8 (amount) + 32 (binding) + 32 (nonce)
    //        + 8 (expiry) + 1 (bump) + 1 (vault_bump)
    pub const MAX_SIZE: usize = 8 + (32 * 3) + 8 + 32 + 32 + 8 + 1 + 1;
}
